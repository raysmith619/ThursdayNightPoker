...
Copyright 2017 Charles Raymond Smith

Based on Paul Griffith's hand.py
...
from PokerCardBase import PokerCardBase, get_rank_integer
...
class PokerHandBase(object):
        ...
        if numcards and deck and not namelist:
            ...
            for name in namelist:
                if tR("hand"):
                    ...
                    names_str += name
                    if name in names:
                        print("Duplicate card {} in hand so far:{}".format(name, names_str))
                        print("Problem here")
                    names[name] += 1
    ...
    def trace(self, flag, *levels):
        """ Selective tracing for debugging / diagnostics
        """
        return self._trace.trace(flag, levels)


    def copy(self):

        """Returns a new PokerHandBase instance which is a copy of the
        original PokerHandBase instance. Copies are made of the Card
    ...
    def exchange(self, chg):
        ...
        for idx in chg:
            if int(idx) in range(len(self._cards)):
                ...
                print("draw:{} exchange".format(self._cards[int(idx)-1]))
    ...
    # cmp - public comparison operator
    # Reverse result if direction == LOW
    def cmp(self, other):
        res = 0
        if self > other:
            res = 1
            ...
            res = -1
        if self.direction == PokerHandDirection.LOW:
            res = -res
        return res
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, PokerHandBase):
    ...
    def __add__(self, other):
        ...
        """Adds the cards in one PokerHandBase to another."""

        if not isinstance(other, PokerHandBase):
            raise NotImplementedError("You can only add another " +
                                      "PokerHandBase to a PokerHandBase.")
        else:
            new_hand = self.copy()
    ...
    def __mul__(self, other):
        ...
        """Multiples the cards in a PokerHandBase by an integer."""

        if not isinstance(other, int) or other < 1:
            raise NotImplementedError("You can only multiply a PokerHandBase " +
    ...
    def __rmul__(self, other):
        ...
        if not isinstance(other, int) or other < 1:
            raise NotImplementedError("You can only multiply a " +
                                      "PokerHandBase by a positive integer")
        else:
            new_cards = []
            for copies in range(other):
                new_cards.extend(self.get_list())
    ...
    def __imul__(self, other):
        ...
        if not isinstance(other, int) or other < 1:
            raise NotImplementedError("You can only multiply a PokerHandBase by " +
                                      "a positive integer")
        else:
            new_cards = []
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, slice):
            ...
            return PokerHandBase(cardlist=new_list, nocopy=True)
        else:
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(key, slice):
            if not isinstance(value, PokerHandBase):
                raise TypeError("Only other PokerHandBase instances can be " +
                                "assigned via slicing.")
            self._cards[key] = value.get_list()
            self._cards_changed()
        elif not isinstance(value, PokerCardBase):
    ...
    def __contains__(self, item):
        ...
        if not isinstance(item, PokerCardBase):
            return False
        else:
    ...
    def append(self, value):
        ...
        if not isinstance(value, PokerCardBase):
            raise TypeError("Only PokerCardBase instances can be appended.")
        else:
    ...
    def count(self, value):
        ...
        if isinstance(value, PokerCardBase):
    ...
    def extend(self, hand):
        ...
        """Extends the card list with that of another PokerHandBase."""

        if not isinstance(hand, PokerHandBase):
            raise TypeError("PokerHandBase instances may only be extended with " +
                            "other PokerHandBase instances.")
        else:
    ...
    def index(self, value):
        ...
        if isinstance(value, PokerCardBase):
            for idx, card in enumerate(self._cards):
                ...
                raise ValueError("PokerCardBase.index(x): x not in PokerCardBase")
        elif isinstance(value, int) or isinstance(value, basestring):
            ...
            raise ValueError("PokerCardBase.index(c): x not in PokerCardBase")
    ...
    def insert(self, idx, value):
        ...
        if not isinstance(value, PokerCardBase):
            raise TypeError("Only PokerCardBase instances may be inserted into " +
                            "PokerHandBase instances.")
        else:
    ...
    def remove(self, value):
        ...
        """Removes the first PokerCardBase from the card list whose value is
        'value'.

        """

        if not isinstance(value, PokerCardBase):
            raise TypeError("Only PokerCardBases may be removed from a PokerHandBase.")
        else:
    ...
    def sort(self, key=None, reverse=False):        # pylint: disable=W0613
        ...
        self._cards.sort(key=PokerCardBase._sort_index, reverse=reverse)
